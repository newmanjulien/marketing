...
>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-[1fr_minmax(0,0.9fr)]">
    <div class="hidden flex-col gap-[16px] px-[16px] py-[16px] sm:flex">
      {#each connectionSettings as setting (setting.label)}
        <div class="flex items-center justify-between gap-4">
          ...
          <span
            class="min-w-[112px] rounded-[6px] border border-stone-200 bg-stone-50 px-[14px] py-[9px] text-center text-[14px] leading-none tracking-normal text-stone-600"
          >
            {setting.value}
          </span>
        </div>
    ...
    <div
      class={[
        'flex items-center justify-center bg-stone-50/60 px-[18px] py-[18px] sm:border-l sm:border-stone-200 sm:px-[22px] sm:py-[22px]',
        onNext && 'pt-[54px] sm:pt-[22px]'
      ]}
    >
