...
<div class="rounded-[16px] border border-stone-200 bg-white px-[18px] py-[18px] shadow-[0_1px_0_rgba(48,47,45,0.03)] sm:px-[22px] sm:py-[22px]">
  <div class="flex items-center justify-between gap-4">
    ...
    <span
      class="shrink-0 rounded-[6px] bg-stone-100 px-[9px] py-[6px] font-mono text-[14px] leading-none tracking-normal text-stone-600"
    >
      {invited} invited
    </span>
  </div>

  <div class="mt-[14px] -mx-[8px] flex flex-col gap-[6px] sm:-mx-[10px]">
    {#each stages as row (row.label)}
      {@const isHighlighted = row === highlighted}
      <div
        class={[
          'rounded-[10px] border px-[14px] py-[13px]',
      ...
      >
        <div class="flex items-center justify-between gap-[12px]">
          <span
            class={[
              'truncate text-[16px] font-book leading-none tracking-normal',
        ...
        <p class="mt-[10px] font-mono text-[14px] leading-none tracking-normal text-stone-400">
          {row.detail}
        </p>

        <div class="mt-[13px] h-[7px] overflow-hidden rounded-full bg-stone-100" aria-hidden="true">
          <div
            class={['h-full rounded-full', isHighlighted ? 'bg-stone-300' : 'bg-stone-200']}
