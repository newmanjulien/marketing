<script lang="ts" module>
  ...
  export type NetworkResultsData = Record<RowId, { count: string; rate: string }>;
...
<div class="rounded-[16px] border border-stone-200 bg-white p-[18px] shadow-[0_1px_0_rgba(48,47,45,0.03)] sm:p-[22px]">
  <div class="relative">
    <p class="text-[16px] font-book leading-none text-stone-500">
      Your practice
    </p>
    ...
    >
      <defs>
        <linearGradient id="{uid}-line-fade" x1="0" y1="0" x2="300" y2="0" gradientUnits="userSpaceOnUse">
  ...
  >
    {#each resultRows as row, index (row.id)}
      ...
      >
        ...
        <span
          class={[
            'flex-1 truncate text-[16px] font-book leading-none transition-colors',
        ...
        <span class="shrink-0 text-[15px] leading-none text-stone-400">
          {row.count}
        </span>

        <span
          class={[
            'shrink-0 rounded-[6px] py-[6px] text-[14px] leading-none transition-colors',
