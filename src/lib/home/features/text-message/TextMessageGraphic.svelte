    ...
    <div class="min-h-0 flex-1 overflow-auto px-[18px] py-[22px] sm:px-[24px]">
      <div class="flex flex-col items-start gap-[3px]">
        {#each messages as message, index}
          <div
            class="bubble {index === messages.length - 1 ? 'bubble-tail' : ''} max-w-[82%] whitespace-pre-wrap break-words px-[14px] py-[8px] text-left font-body text-[15px] font-book leading-[1.35] tracking-normal text-white sm:text-[16px]"
          >{message}</div>
        {/each}
      </div>
    </div>
